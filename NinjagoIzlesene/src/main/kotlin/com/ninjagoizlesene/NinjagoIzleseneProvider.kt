    ...
    override suspend fun loadLinks(
        ...
        for (playerUrl in playerPageUrls.distinct()) {
            ...
            for (iframeUrl in iframeUrls) {
                ...
                // yedeğine düşüyoruz.
                val isYoutube = iframeUrl.contains("youtube.com/embed/")
                var linksBeforeCall = 0
                val countingCallback: ((ExtractorLink) -> Unit) =
                    if (isYoutube) { link -> linksBeforeCall++; callback(link) } else callback
            ...
            Regex(
                ...
                .forEach { embedUrl ->
                    ...
                    val isYoutube = embedUrl.contains("youtube.com/embed/")
                    var linksBeforeCall = 0
                    val countingCallback: ((ExtractorLink) -> Unit) =
                        if (isYoutube) { link -> linksBeforeCall++; callback(link) } else callback
