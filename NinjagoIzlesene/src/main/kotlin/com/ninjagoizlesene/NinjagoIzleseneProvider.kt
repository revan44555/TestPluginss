...
class NinjagoIzleseneProvider : MainAPI() {
    ...
    private suspend fun driveListChildren(folderId: String): List<DriveFile> {
        return runCatching {
            ...
            Log.d("NinjagoIzlesene", "Drive klasör taraması başarısız ($folderId): ${it.message}")
    ...
    private fun parseSeasonNumberFromFolderName(name: String): Int? {
        ...
        for (pattern in patterns) {
            pattern.find(name)?.let { return it.groupValues[1].toIntOrNull() }
        }
        return null
    }
    ...
    private suspend fun findDriveSeasonFolderId(season: Int): String? {
        if (driveSeasonFolderCache.containsKey(season)) return driveSeasonFolderCache[season]
        if (!driveRootScanned) {
            ...
            for (child in children) {
                ...
                val seasonNum = parseSeasonNumberFromFolderName(child.name) ?: continue
    ...
    private suspend fun resolveDriveEpisode(season: Int, episode: Int): String? {
        val seasonFolderId = findDriveSeasonFolderId(season) ?: return null
        val files = driveListChildren(seasonFolderId)
        val match = files.firstOrNull {
            it.mimeType != "application/vnd.google-apps.folder" &&
                parseEpisodeNumberFromFileName(it.name) == episode
        } ?: return null
        return "https://drive.google.com/file/d/${match.id}/view"
    }
    ...
    override suspend fun loadLinks(
        ...
        run {
            val match = urlSeasonEpisodeRegex.find(pageUrl) ?: return@run
            val season = match.groupValues[1].toIntOrNull() ?: return@run
            val episode = match.groupValues[2].toIntOrNull() ?: return@run
            val driveLink = runCatching { resolveDriveEpisode(season, episode) }
                .onFailure {
                    Log.d("NinjagoIzlesene", "Drive çözümleme istisna fırlattı ($season-$episode): ${it.message}")
                }
                .getOrNull()
            if (driveLink != null) {
                val extracted = runCatching {
                    ...
                    Log.d("NinjagoIzlesene", "Drive linki extractor'a takıldı ($driveLink): ${it.message}")
                }.getOrDefault(false)
