...
class NinjagoIzleseneProvider : MainAPI() {
    ...
    private suspend fun resolveDriveEpisode(season: Int, episode: Int): String? {
        ...
        val link = "https://www.googleapis.com/drive/v3/files/${match.id}?alt=media&key=$driveApiKey"
    ...
    // NOT: Sezon 11+ akışında YouTube/Piped çözümü artık denenmiyor (bkz. loadLinks).
    // Bu mantık muhtemelen sezon 1-10 için (Drive akışından önce) tasarlanmıştı ve
    // canlı sitede (sezon 11+) hatalı/gereksiz denemelere yol açıyordu.
    ...
    override suspend fun loadLinks(
        ...
        // Sezon 1-10: sadece Google Drive kaynağı kullanılır (bu sezonlar için
        // YouTube embed'i güvenilmez şekilde çalışmıyor). Drive'da bölüm yoksa
        // siteye/YouTube'a hiç düşülmez, direkt "bulunamadı" ile sonuçlanır.
        // Sezon 11+: Drive'a hiç bakılmaz, direkt site akışına (Dzen vb.) geçilir.
        run {
            ...
            if (season !in 1..10) {
                Log.d("NinjagoIzlesene", "DRIVE-DEBUG: sezon $season 1-10 aralığı dışında, Drive atlanıyor, site akışına devam")
                return@run
            }
            ...
            if (driveLink != null) {
                Log.d("NinjagoIzlesene", "DRIVE-DEBUG: direkt link callback'e veriliyor -> $driveLink")
                val delivered = runCatching {
                    ...
                    Log.d("NinjagoIzlesene", "DRIVE-DEBUG: callback İSTİSNA fırlattı ($driveLink): ${it::class.simpleName}: ${it.message}")
                }.getOrDefault(false)
                Log.d("NinjagoIzlesene", "DRIVE-DEBUG: callback teslim sonucu = $delivered")
                if (delivered) {
                    Log.d("NinjagoIzlesene", "DRIVE-DEBUG: Drive linki BAŞARILI, true ile dönülüyor")
                    return true
                }
            }
            // Sezon 1-10 aralığındayken Drive'da bölüm bulunamadıysa (ya da
            // teslim başarısız olduysa) siteye/YouTube'a HİÇ düşülmez.
            Log.d("NinjagoIzlesene", "DRIVE-DEBUG: sezon $season 1-10 aralığında ama Drive'da bölüm $episode yok, false ile sonlandırılıyor")
            return false
        }
        ...
        for (playerUrl in playerPageUrls.distinct()) {
            ...
            for (iframeUrl in iframeUrls) {
                ...
                // YouTube: sezon 11+ akışında artık denenmiyor (bkz. dosya başındaki not).
                val isYoutube = iframeUrl.contains("youtube.com/embed/", ignoreCase = true) ||
                    iframeUrl.contains("youtu.be/", ignoreCase = true)
                if (isYoutube) {
                    Log.d("NinjagoIzlesene", "YouTube iframe atlandı (denenmiyor): $iframeUrl")
                } else {
                    runCatching {
            ...
            // YouTube: sezon 11+ akışında artık denenmiyor, sadece loglanıp atlanıyor.
            Regex(
                """https?://(?:www\.)?youtube\.com/embed/[^"'\\\s<>]+"""
            ).findAll(playerHtml).map { it.value }.distinct()
                .forEach { embedUrl ->
                    Log.d("NinjagoIzlesene", "Gömülü YouTube linki atlandı (denenmiyor): $embedUrl")
                }
            Regex(
