...
class NinjagoIzleseneProvider : MainAPI() {
    ...
    private suspend fun fetchTelegramIndex(maxPages: Int = 5): List<TgPost> {
        ...
        while (url != null && pages < maxPages) {
            ...
            val response = fetchResult.getOrNull()
            if (response == null) {
                Log.d(
                    "NinjagoIzlesene",
                    "Telegram: '$url' açılamadı, istisna: ${fetchResult.exceptionOrNull()?.message}"
                )
                break
            }
            if (!response.isSuccessful) {
                Log.d(
                    "NinjagoIzlesene",
                    "Telegram: '$url' HTTP ${response.code} döndü"
                )
                break
            }
            ...
            if (messages.isEmpty()) {
                Log.d(
                    "NinjagoIzlesene",
                    "Telegram: '$url' sayfasında .tgme_widget_message bulunamadı " +
                        "(htmlLen=${doc.html().length}, bodyStart=${doc.html().take(200).replace("\n", " ")})"
                )
                break
            }
            var minId = Int.MAX_VALUE
            for (m in messages) {
                val rawDataPost = m.attr("data-post").ifBlank {
                    m.closest("[data-post]")?.attr("data-post") ?: ""
                }
                val id = rawDataPost.substringAfter('/').toIntOrNull()
                if (id == null) {
                    Log.d(
                        "NinjagoIzlesene",
                        "Telegram: mesajda data-post okunamadı (rawDataPost='$rawDataPost'), atlanıyor"
                    )
                    continue
                }
                ...
                val videoSrc = m.selectFirst("video")?.attr("src")?.takeIf { it.isNotBlank() }
                    ...
                    ?: m.selectFirst("[data-src]")?.attr("data-src")?.takeIf { it.isNotBlank() }
                val captionText = m.selectFirst(".tgme_widget_message_text")?.text()
                    ?.takeIf { it.isNotBlank() } ?: m.text()
                Log.d(
                    "NinjagoIzlesene",
                    "Telegram: gönderi #$id metin='${captionText.take(60)}' " +
                        "videoTagVarMı=${m.selectFirst("video") != null} videoSrc=${videoSrc ?: "YOK"}"
                )
                tgCache.add(TgPost(id, captionText, videoSrc))
            }
            if (minId == Int.MAX_VALUE) break
            url = "https://t.me/s/$telegramChannel?before=$minId"
        }
        tgScanned = true
        Log.d("NinjagoIzlesene", "Telegram: tarama bitti, toplam ${tgCache.size} gönderi önbelleğe alındı")
        return tgCache
    }
    ...
    private suspend fun findTelegramVideo(pageUrl: String): String? {
        ...
        val m = telegramEpisodeRegex.find("/$rawPath/")
        if (m == null) {
            Log.d("NinjagoIzlesene", "Telegram: '$pageUrl' bölüm URL desenine uymadı, atlanıyor")
            return null
        }
        ...
        if (matches.isEmpty()) {
            Log.d(
                "NinjagoIzlesene",
                "Telegram: '$label' için eşleşme yok (önbellekte ${index.size} gönderi var)"
            )
            return null
        }
