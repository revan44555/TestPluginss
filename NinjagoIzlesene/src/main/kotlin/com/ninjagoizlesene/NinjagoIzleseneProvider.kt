...
class NinjagoIzleseneProvider : MainAPI() {
    ...
    override suspend fun loadLinks(
        ...
        if (path.startsWith("dizi/")) {
            // slug = "dizi/<slug>/..." içindeki <slug> kısmı.
            val slug = path.removePrefix("dizi/").substringBefore("/")
            // Artık "içinde rakam var mı" gibi belirsiz bir metin taraması
            // DEĞİL, doğrudan isEpisodeLink (URL yapısı) kullanıyoruz.
            // Böylece sayfadaki tür-filtresi/yorum/navigasyon linkleri
            // (hepsi de rakam içerebiliyordu) artık yanlışlıkla seçilmiyor.
            val episodeUrl = doc.select("a[href]")
                .map { fixUrl(it.attr("href").trim()) }
                .firstOrNull { it != pageUrl && isEpisodeLink(it, slug) }
            if (episodeUrl != null) {
                pageUrl = episodeUrl
