...
class NinjagoIzleseneProvider : MainAPI() {
    ...
    override suspend fun loadLinks(
        ...
        // Site "Oynatıcı 1 / Oynatıcı 2" gibi birden fazla player sekmesi
        // sunuyor; ilk iframe genelde about:blank (JS tıklamayla dolduruluyor),
        // gerçek player URL'leri (ör. youtube.com/embed/...) sayfanın script
        // içeriğine (data attribute, JSON, JS değişkeni) gömülü olabilir.
        // Bu yüzden ham HTML üzerinden de embed/player URL'lerini tarıyoruz.
        Regex(
            """https?://(?:www\.)?youtube\.com/embed/[^"'\\\s<>]+"""
        ).findAll(html).map { it.value }.distinct()
            .forEach { ytUrl ->
                runCatching {
                    loadExtractor(ytUrl, pageUrl, subtitleCallback, callback)
                }.onSuccess { success ->
                    if (success) foundLink = true
                }
            }
