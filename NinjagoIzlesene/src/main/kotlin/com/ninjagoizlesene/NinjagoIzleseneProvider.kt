...
class NinjagoIzleseneProvider : MainAPI() {
    ...
    // ---------------------------------------------------------------
    // CloudStream'in YouTube extractor'ı standart video URL'siyle daha
    // güvenilir çalışabildiği için embed URL'sini önce watch?v= biçimine
    // dönüştürüyoruz. Playlist parametreleri video kimliğinden ayrılır.
            ...
            for (iframeUrl in iframeUrls) {
                ...
                if (isYoutube) {
                    if (tryYoutubeExtractor(iframeUrl, playerUrl, subtitleCallback, callback)) {
                        ...
                        // Harici proxy yerine son çare olarak mevcut Piped fallback'i
                        // kullanılır. Ana yol her zaman CloudStream extractor'ıdır.
                        runCatching {
                    ...
                    if (isYoutube) {
                        if (tryYoutubeExtractor(embedUrl, playerUrl, subtitleCallback, callback)) {
                            ...
                            runCatching {
                                ...
                                Log.d("NinjagoIzlesene", "Gömülü YouTube fallback başarısız ($embedUrl): ${it.message}")
                            }
                        }
                    } else {
                        runCatching {
