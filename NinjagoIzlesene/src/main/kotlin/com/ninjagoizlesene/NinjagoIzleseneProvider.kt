...
class NinjagoIzleseneProvider : MainAPI() {
    ...
    private val headers = mapOf(
        ...
        "Referer" to "https://ninjagoizlesene.com.tr/"
    )

    private fun extractTitle(link: Element, fallbackSlug: String): String {
        var title = link.text().replace(Regex("\\s+"), " ").trim()
    ...
    // TEŞHIS AMAÇLI: arama hiçbir sonuç bulamazsa, hatanın gerçek
    // sebebini (HTTP kodu, toplam <a> sayısı, /dizi/ eşleşme sayısı)
    // CloudStream'in "Günlük" ekranında görebilmemiz için exception
    // mesajına gömüyoruz.
    override suspend fun search(query: String): List<SearchResponse> {
        val response = app.get("$mainUrl/dizi-arsivi/", headers = headers)
        ...
        // Hâlâ boşsa: teşhis bilgisini hata olarak fırlat, böylece
        // CloudStream'in "Günlük" ekranında gerçek sebebi görebiliriz.
        if (results.isEmpty()) {
            throw Exception(
                "TESHIS: httpCode=${response.code} " +
                "totalLinks=${allLinks.size} " +
                "diziLinks=${seriesLinks.size} " +
                "bodyLen=${response.text.length} " +
                "bodyStart=${response.text.take(150).replace("\n", " ")}"
            )
        }
